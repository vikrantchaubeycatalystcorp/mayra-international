...
export default function MockTestsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F4]">
      {/* Hero Section */}
      <div className="relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-600 to-primary-800 px-4 py-16">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        {isReturningUser && (
          <div className="mb-10 space-y-6">
            ...
            <div className="rounded-2xl bg-gradient-to-r from-primary-50 to-primary-50 border border-primary-100 p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  {stats.currentStreak > 0 && (
                    <div className="flex items-center gap-2 rounded-lg bg-accent-100 px-3 py-1.5">
                      <Flame className="h-4 w-4 text-accent-600" />
                      <span className="text-sm font-bold text-accent-700">{stats.currentStreak} day streak</span>
                    </div>
                  )}
                  ...
                  {recentAttempts.map((a) => (
                    <div key={a.id} className="flex items-center gap-3 rounded-xl bg-white p-3 border border-gray-100">
                      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-sm font-black ${
                        ...
                        : a.percentage >= 40 ? "bg-accent-100 text-accent-700"
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  {recommendations.map((rec) => (
                    <Link key={rec.examId} href={`/mock-tests/${rec.examSlug}`}>
                      <div className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:border-primary-200 hover:-translate-y-0.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5 mb-0.5">
                              <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase ${
                                ...
                                : rec.priority === "medium" ? "bg-accent-100 text-accent-600"
                            ...
                            {rec.focusAreas.length > 0 && (
                              <div className="mt-2 flex flex-wrap gap-1">
                                {rec.focusAreas.slice(0, 3).map((area) => (
                                  <span key={area} className="rounded-md bg-primary-50 px-1.5 py-0.5 text-[9px] font-medium text-primary-600">
                                    {area}
                                  </span>
                                ))}
                              </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200"
            />
          </div>
          <div className="relative">
            ...
            <select
              ...
              className="appearance-none rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-10 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200"
            >
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { icon: BarChart3, label: "AI Diagnostics", desc: "Know exactly why your score happened", color: "text-primary-600", bg: "bg-primary-100" },
            { icon: Zap, label: "Speed Analysis", desc: "Track time per question and pace", color: "text-accent-600", bg: "bg-accent-100" },
            { icon: Star, label: "Smart Review", desc: "Filter by careless mistakes, guesses", color: "text-emerald-600", bg: "bg-emerald-100" },
            { icon: Target, label: "Personalized", desc: "AI picks your next best test", color: "text-primary-600", bg: "bg-primary-100" },
          ].map((f) => (
        ...
        {/* CTA Banner */}
        <div className="mt-12 rounded-3xl bg-gradient-to-r from-primary-600 to-primary-600 p-8 text-white text-center">
          <h2 className="text-2xl font-black">Ready to Top the Leaderboard?</h2>
          <p className="mt-2 text-primary-200 max-w-lg mx-auto">
