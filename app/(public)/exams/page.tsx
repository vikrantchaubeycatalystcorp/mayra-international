...
type LevelFilter = "All" | "UG" | "PG" | "PhD" | "Diploma";
type StreamFilter = "All" | "Engineering" | "Medical" | "Management" | "Law" | "Defence";

const levelFilters: LevelFilter[] = ["All", "UG", "PG", "PhD", "Diploma"];
const streamFilters: StreamFilter[] = ["All", "Engineering", "Medical", "Management", "Law", "Defence"];
    ...
    <article className="bg-white rounded-2xl border border-gray-100 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 p-5">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        ...
        <div className="p-2.5 bg-gray-50 rounded-xl">
          ...
          <p className="text-xs font-bold text-gray-800 mt-0.5">
            ₹{exam.applicationFee.general.toLocaleString()}
          </p>
        </div>
      </div>
...
export default function ExamsPage() {
  ...
  const filtered = useMemo(() => {
    return exams.filter((exam) => {
      ...
      const matchStream =
        stream === "All" || exam.stream.includes(stream);
