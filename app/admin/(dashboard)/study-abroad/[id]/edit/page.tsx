...
export default function EditStudyAbroadPage() {
  ...
  useEffect(() => {
    async function load() {
      const data = await fetchOne<Record<string, unknown>>("/api/admin/study-abroad", id);
      if (data) {
        setForm({
          name: String(data.name || ""), flag: String(data.flag || ""),
          universities: Number(data.universities || 0), avgCost: String(data.avgCost || ""),
          popularCourses: (data.popularCourses as string[]) || [],
          description: String(data.description || ""),
          topUniversities: (data.topUniversities as { name: string; rank?: number }[]) || [],
          whyStudyHere: String(data.whyStudyHere || ""),
          visaInfo: String(data.visaInfo || ""),
          scholarships: String(data.scholarships || ""),
          livingCost: String(data.livingCost || ""),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateItem("/api/admin/study-abroad", id, form);
    if (result.success) router.push("/admin/study-abroad"); else setError(result.error || "Failed to update");
    setLoading(false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={labelClass}>Country Name *</label><input type="text" value={form.name} onChange={(e) => update("name", e.target.value)} required className={inputClass} /></div>
            <div><label className={labelClass}>Flag Emoji *</label><input type="text" value={form.flag} onChange={(e) => update("flag", e.target.value)} required className={inputClass} /></div>
            ...
            <div><label className={labelClass}>Average Cost *</label><input type="text" value={form.avgCost} onChange={(e) => update("avgCost", e.target.value)} required className={inputClass} /></div>
          ...
          <div><label className={labelClass}>Description</label><textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={4} className={textareaClass} /></div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          <div><label className={labelClass}>Why Study Here</label><textarea value={form.whyStudyHere} onChange={(e) => update("whyStudyHere", e.target.value)} rows={3} className={textareaClass} /></div>
          <div><label className={labelClass}>Visa Information</label><textarea value={form.visaInfo} onChange={(e) => update("visaInfo", e.target.value)} rows={3} className={textareaClass} /></div>
          <div><label className={labelClass}>Scholarships</label><textarea value={form.scholarships} onChange={(e) => update("scholarships", e.target.value)} rows={3} className={textareaClass} /></div>
          <div><label className={labelClass}>Living Cost</label><textarea value={form.livingCost} onChange={(e) => update("livingCost", e.target.value)} rows={2} className={textareaClass} /></div>
        </div>
