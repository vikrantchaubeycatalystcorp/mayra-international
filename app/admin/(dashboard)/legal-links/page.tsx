...
import { Scale, Plus, Edit3, Trash2, X, Loader2, Search } from "lucide-react";
...
const EMPTY_FORM: FormData = { label: "", href: "", sortOrder: 0, isActive: true };

export default function AdminLegalLinksPage() {
  const [items, setItems] = useState<LegalLink[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteTarget, setDeleteTarget] = useState<LegalLink | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/legal-links", { credentials: "include" });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/legal-links/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
    } finally { setDeleting(false); setDeleteTarget(null); }
  };

  const openCreate = () => { setForm(EMPTY_FORM); setEditId(null); setShowForm(true); };
  const openEdit = (item: LegalLink) => {
    setForm({ label: item.label, href: item.href, sortOrder: item.sortOrder, isActive: item.isActive });
    setEditId(item.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/legal-links/${editId}` : "/api/admin/legal-links";
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Legal Links</h1>
            <p className="text-sm text-gray-500">Manage footer legal and policy links</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No legal links found</p>
          </div>
        ) : (
            ...
            {items.sort((a, b) => a.sortOrder - b.sortOrder).map((item) => (
              <div key={item.id} className="flex items-center justify-between px-4 py-3 hover:bg-blue-50/30 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{item.href} &middot; Order: #{item.sortOrder}</p>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Legal Link" : "Add Legal Link"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Label</label><input className={inputClass} value={form.label} onChange={(e) => set("label", e.target.value)} required placeholder="e.g. Privacy Policy" /></div>
              <div><label className={labelClass}>Href</label><input className={inputClass} value={form.href} onChange={(e) => set("href", e.target.value)} required placeholder="/privacy-policy" /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Legal Link" message={`Delete "${deleteTarget?.label}"?`} loading={deleting} />
    </div>
  );
}
