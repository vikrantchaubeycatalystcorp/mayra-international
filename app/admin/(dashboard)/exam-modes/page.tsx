...
import { Monitor, Plus, Edit3, Trash2, X, Loader2, Search } from "lucide-react";
...
export default function AdminExamModesPage() {
  const [items, setItems] = useState<ExamMode[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteTarget, setDeleteTarget] = useState<ExamMode | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/exam-modes", { credentials: "include" });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/exam-modes/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
    } finally { setDeleting(false); setDeleteTarget(null); }
  };

  const openCreate = () => { setForm(EMPTY_FORM); setEditId(null); setShowForm(true); };
  const openEdit = (item: ExamMode) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/exam-modes/${editId}` : "/api/admin/exam-modes";
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Exam mode updated." : "Exam mode created." });
        setShowForm(false);
        fetchData();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Exam Modes</h1>
            <p className="text-sm text-gray-500">Manage exam mode types (Online, Offline, Hybrid)</p>
          </div>
        </div>
        <button onClick={openCreate} className="inline-flex items-center gap-2 h-10 px-5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-blue-500/25 active:scale-[0.98]">
          <Plus className="w-4 h-4" /> Add Exam Mode
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No exam modes found</p>
          </div>
        ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Exam Mode" : "Add Exam Mode"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Name</label><input className={inputClass} value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. Online" /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Exam Mode" message={`Delete "${deleteTarget?.name}"? This may affect associated exams.`} loading={deleting} />
    </div>
  );
}
