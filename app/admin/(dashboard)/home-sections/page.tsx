...
const EMPTY_FORM: FormData = { sectionKey: "", title: "", subtitle: "", isActive: true };

export default function AdminHomeSectionsPage() {
  const crud = useAdminCRUD<HomeSection>({ endpoint: "/api/admin/home-sections" });
  ...
  const openEdit = (item: HomeSection) => {
    setForm({ sectionKey: item.sectionKey, title: item.title, subtitle: item.subtitle || "", isActive: item.isActive });
    setEditId(item.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/home-sections/${editId}` : "/api/admin/home-sections";
      ...
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Section updated." : "Section created." });
        setShowForm(false);
        crud.refetch();
      } else {
  ...
  const columns: Column<HomeSection>[] = [
    {
      ...
      label: "Section Key",
      sortable: true,
      render: (item) => <span className="text-xs font-mono font-medium text-purple-700 bg-purple-50 px-2 py-1 rounded">{item.sectionKey}</span>,
    },
    {
      key: "title",
      label: "Title",
      sortable: true,
      render: (item) => <p className="text-sm font-semibold text-gray-900 truncate max-w-[250px]">{item.title}</p>,
    },
    {
      key: "subtitle",
      label: "Subtitle",
      render: (item) => <p className="text-sm text-gray-500 truncate max-w-[200px]">{item.subtitle || "—"}</p>,
    },
    {
      key: "isActive",
      label: "Status",
  ...
  return (
    <>
      ...
      <AdminDataTable
        title="Home Sections"
        description="Manage homepage section titles and visibility"
        columns={columns}
        data={crud.data}
        total={crud.total}
        page={crud.page}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Section" : "Add Section"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Section Key</label><input className={inputClass} value={form.sectionKey} onChange={(e) => set("sectionKey", e.target.value)} required placeholder="e.g. featured_courses" /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Section" message={`Delete "${deleteTarget?.title}"?`} loading={deleting} />
    </>
  );
}
