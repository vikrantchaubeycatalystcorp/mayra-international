...
import { Building2, Plus, Edit3, Trash2, X, Loader2, Search } from "lucide-react";
...
export default function AdminRecruitersPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/recruiters/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
    } finally { setDeleting(false); setDeleteTarget(null); }
  };

  const openCreate = () => { setForm(EMPTY_FORM); setEditId(null); setShowForm(true); };
  const openEdit = (item: Recruiter) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/recruiters/${editId}` : "/api/admin/recruiters";
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Recruiter updated." : "Recruiter created." });
        setShowForm(false);
        fetchData();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Top Recruiters</h1>
            <p className="text-sm text-gray-500">Manage the recruiter library shown on college pages</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No recruiters found</p>
          </div>
        ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Recruiter" : "Add Recruiter"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Name</label><input className={inputClass} value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. Google" /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Recruiter" message={`Delete "${deleteTarget?.name}"? It will be removed from all colleges.`} loading={deleting} />
    </div>
  );
}
