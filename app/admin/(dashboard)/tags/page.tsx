...
import { Tag, Plus, Edit3, Trash2, X, Loader2, Search } from "lucide-react";
...
const EMPTY_FORM: FormData = { name: "", slug: "" };
...
export default function AdminTagsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/tags/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Tag updated." : "Tag created." });
        setShowForm(false);
        fetchData();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage content tags</p>
          </div>
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Tag" : "Add Tag"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Name</label><input className={inputClass} value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. Top Rated" /></div>
              <div><label className={labelClass}>Slug</label><input className={inputClass} value={form.slug} onChange={(e) => set("slug", e.target.value)} required placeholder="top-rated" /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Tag" message={`Delete "${deleteTarget?.name}"? This may affect tagged content.`} loading={deleting} />
    </div>
  );
}
