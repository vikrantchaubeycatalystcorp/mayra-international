...
import { Database, Plus, Edit3, Trash2, X, Loader2, Search } from "lucide-react";
...
const EMPTY_FORM: FormData = { name: "", type: "API", url: "", status: "active", notes: "", isActive: true };

const TYPE_OPTIONS = ["API", "Scraper", "Manual", "Feed", "Partner", "Other"];
const STATUS_OPTIONS = ["active", "inactive", "error", "maintenance"];
...
export default function AdminDataSourcesPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/data-sources/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
  ...
  const openEdit = (item: DataSource) => {
    setForm({ name: item.name, type: item.type, url: item.url || "", status: item.status, notes: item.notes || "", isActive: item.isActive });
    setEditId(item.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/data-sources/${editId}` : "/api/admin/data-sources";
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Data source updated." : "Data source created." });
        setShowForm(false);
        fetchData();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage external data sources and integrations</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No data sources found</p>
          </div>
        ) : (
              ...
              <div key={item.id} className="flex items-center justify-between px-4 py-3 hover:bg-blue-50/30 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Type: {item.type}
                    {item.url && <> &middot; <span className="text-blue-400">{item.url}</span></>}
                  </p>
                  {item.notes && <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{item.notes}</p>}
                </div>
                <div className="flex items-center gap-2">
                  <StatusBadge status={statusColor(item.status) as "active" | "inactive"} label={item.status} />
                  <StatusBadge status={item.isActive ? "active" : "inactive"} label={item.isActive ? "Enabled" : "Disabled"} />
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Data Source" : "Add Data Source"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Name</label><input className={inputClass} value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. NIRF Rankings API" /></div>
              ...
              <div><label className={labelClass}>URL</label><input className={inputClass} value={form.url} onChange={(e) => set("url", e.target.value)} placeholder="https://api.example.com" /></div>
              <div><label className={labelClass}>Notes</label><textarea className={`${inputClass} h-20 py-2`} value={form.notes} onChange={(e) => set("notes", e.target.value)} placeholder="Additional notes..." /></div>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.isActive} onChange={(e) => set("isActive", e.target.checked)} className="rounded border-gray-300" /> Enabled
              </label>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Data Source" message={`Delete "${deleteTarget?.name}"? This cannot be undone.`} loading={deleting} />
    </div>
  );
}
