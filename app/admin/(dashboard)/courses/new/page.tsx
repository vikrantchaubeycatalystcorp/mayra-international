...
export default function NewCoursePage() {
  ...
  const [form, setForm] = useState({ name: "", stream: "", level: "UG", duration: "", description: "", topColleges: 10, avgFees: 100000, avgSalary: null as number | null, icon: "", color: "#3B82F6", isFeatured: false, isActive: true });

  const update = (key: string, value: unknown) => setForm((p) => ({ ...p, [key]: value }));
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setError(""); setLoading(true);
    const result = await createItem("/api/admin/courses", form);
    if (result.success) router.push("/admin/courses"); else setError(result.error || "Failed to create");
    setLoading(false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2"><label className={labelClass}>Course Name *</label><input type="text" value={form.name} onChange={(e) => update("name", e.target.value)} required className={inputClass} placeholder="e.g., B.Tech Computer Science" /></div>
