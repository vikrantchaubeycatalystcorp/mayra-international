...
const EMPTY_FORM: FormData = { name: "", fullName: "", description: "", sortOrder: 0 };
...
export default function AdminAccreditationsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/accreditations", { credentials: "include" });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/admin/accreditations/${deleteTarget.id}`, { method: "DELETE", credentials: "include" });
      await fetchData();
  ...
  const openEdit = (item: Accreditation) => {
    setForm({ name: item.name, fullName: item.fullName, description: item.description, sortOrder: item.sortOrder });
    setEditId(item.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const url = editId ? `/api/admin/accreditations/${editId}` : "/api/admin/accreditations";
      ...
      const json = await res.json();
      if (json.success) {
        setMessage({ type: "success", text: editId ? "Accreditation updated." : "Accreditation created." });
        setShowForm(false);
        fetchData();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Accreditations</h1>
            <p className="text-sm text-gray-500">Manage accreditation bodies (NAAC, NBA, etc.)</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No accreditations found</p>
          </div>
        ) : (
            ...
            {items.sort((a, b) => a.sortOrder - b.sortOrder).map((item) => (
              <div key={item.id} className="flex items-center justify-between px-4 py-3 hover:bg-blue-50/30 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{item.fullName} &middot; Order: #{item.sortOrder}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Accreditation" : "Add Accreditation"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className={labelClass}>Short Name</label><input className={inputClass} value={form.name} onChange={(e) => set("name", e.target.value)} required placeholder="e.g. NAAC" /></div>
              <div><label className={labelClass}>Full Name</label><input className={inputClass} value={form.fullName} onChange={(e) => set("fullName", e.target.value)} required placeholder="e.g. National Assessment and Accreditation Council" /></div>
              <div><label className={labelClass}>Description</label><textarea className={`${inputClass} h-20 py-2`} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Brief description..." /></div>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Accreditation" message={`Delete "${deleteTarget?.name}"? This may affect associated colleges.`} loading={deleting} />
    </div>
  );
}
